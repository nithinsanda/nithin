...
import Dashboard from "../pages/Dashboard"; // ✅ local
...
const adminApi = {
  ...
  presets: {
    ...
    create: (formData) => {
      const token = localStorage.getItem('adminToken');
      return api.post('/api/presets', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `Bearer ${token}`
        }
    ...
    update: (id, formData) => {
      const token = localStorage.getItem('adminToken');
      return api.put(`/api/presets/${id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `Bearer ${token}`
        }
    ...
    uploadToDrive: (data) => api.post('/api/presets/upload-to-drive', data),
  },
